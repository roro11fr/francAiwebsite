...
import { Globe, Code2, MessageSquare, Mic, Zap, Brain, Layers, BarChart2, ArrowRight } from 'lucide-react'
import { useLanguage } from '@/context/LanguageContext'
import { useInView } from '@/hooks/useInView'
import { SectionLabel } from '@/components/ui/SectionWrapper'

const iconMap = { Globe, Code2, MessageSquare, Mic, Zap, Brain, Layers, BarChart2 }
  ...
  return (
    <section id="services" className="bg-ink-900 relative overflow-hidden">
      ...
      <div ref={ref as React.RefObject<HTMLDivElement>} className="relative z-10 max-w-screen-xl mx-auto px-6 sm:px-10">
        ...
        <div className={`animate-enter ${isVisible ? 'is-visible' : ''} pt-16 pb-10 border-b border-white/6`}>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            ...
            <p className="text-zinc-500 text-base font-ui font-light leading-relaxed max-w-xs lg:text-right">
              {t.services.subtitle}
            </p>
          </div>
        </div>
        ...
        <div className="pb-10">
          {t.services.items.map((item, i) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-5 sm:gap-8 lg:gap-12 py-7 lg:py-8">

                  <span className="font-display font-extrabold text-violet-900 group-hover:text-violet-600 transition-colors flex-shrink-0 leading-none tabular-nums text-3xl sm:text-4xl lg:text-5xl w-14 sm:w-16 lg:w-20">
                    {String(i + 1).padStart(2, '0')}
                  </span>

                  <div className="flex-1 min-w-0">
                    <h3 className="font-display font-extrabold text-white group-hover:text-violet-300 transition-colors text-xl sm:text-2xl mb-1.5">
                      {item.title}
                    </h3>
