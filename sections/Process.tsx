      ...
      <div ref={ref as React.RefObject<HTMLDivElement>} className="relative z-10 max-w-screen-xl mx-auto px-6 sm:px-10 py-20 lg:py-28">
        ...
        <div className="hidden lg:grid grid-cols-5 gap-6">
          {t.process.steps.map((step, i) => (
            <div
              key={i}
              className={`animate-enter animate-enter-delay-${Math.min(i + 1, 5)} ${isVisible ? 'is-visible' : ''} flex flex-col`}
            >
              <div className="flex items-center gap-0 mb-4">
                <span className="font-display font-extrabold text-[4rem] xl:text-[5rem] text-violet-600 leading-none tabular-nums">
                  {step.number}
                </span>
                {i < t.process.steps.length - 1 && (
                  <div className="flex-1 h-px bg-gradient-to-r from-violet-700/60 to-transparent ml-2 mb-1" />
                )}
              </div>
        ...
        <div className="lg:hidden flex flex-col gap-0">
          {t.process.steps.map((step, i) => (
            <div
              key={i}
              className={`animate-enter animate-enter-delay-${Math.min(i + 1, 5)} ${isVisible ? 'is-visible' : ''} flex gap-6 ${i < t.process.steps.length - 1 ? 'pb-10' : ''}`}
            >
